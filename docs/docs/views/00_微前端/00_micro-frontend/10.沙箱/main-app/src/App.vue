...
const sandboxSnapshot = ref({
  windowValue: "无",
  title: document.title || "未设置",
  storageValue: "无",
});
let sandboxSnapshotTimer = null;
...
function refreshSandboxSnapshot() {
  sandboxSnapshot.value = {
    windowValue: window.__QIANKUN_SANDBOX_LEAK_TEST__ || "无",
    title: document.title || "未设置",
    storageValue: localStorage.getItem("qiankun:sandbox-demo") || "无",
  };
}
...
onMounted(() => {
  ...
  sandboxSnapshotTimer = window.setInterval(refreshSandboxSnapshot, 800);
});

onBeforeUnmount(() => {
  window.removeEventListener("order:notice", handleOrderNotice);
  window.clearInterval(sandboxSnapshotTimer);
});
</script>
...
<template>
  <div class="page-shell">
    <aside class="sidebar">
      ...
      <section class="sidebar-card sandbox-card">
        <span class="eyebrow">沙箱观察</span>
        <h2>主应用真实环境</h2>
        <p>window 字段：{{ sandboxSnapshot.windowValue }}</p>
        <p>页面标题：{{ sandboxSnapshot.title }}</p>
        <p>localStorage：{{ sandboxSnapshot.storageValue }}</p>
      </section>
    </aside>
