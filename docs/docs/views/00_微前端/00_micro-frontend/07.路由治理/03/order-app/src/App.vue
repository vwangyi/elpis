...
const navItems = [
  // [路由处理2:] RouterLink 也只写订单子应用内部路径，最终 URL 会自动带上 /orders base。
  { to: "/list", label: "订单列表" },
  { to: "/detail/2048", label: "订单详情" },
  { to: "/refund/1024", label: "退款处理" },
];

const dialogConfig = computed(() => {
  if (route.path === "/detail/2048") {
    return {
      actionLabel: "修改订单备注",
      title: "修改订单备注",
