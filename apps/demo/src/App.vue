...
import { PieChartOutlined, MailOutlined, YoutubeOutlined } from '@ant-design/icons-vue'
...
const menus = ref([
  ...
  },
  {
    ...
    children: [
      {
        children: undefined,
        icon: () => h(Checkbox, { checked: checked, 'onUpdate:checked': onUpdateChecked }),
        key: '基本信息',
        label: '基本信息',
        type: undefined,
        disabled: true,
      ...
      {
        children: undefined,
        icon: () => h(Checkbox, { checked: checked2, 'onUpdate:checked': onUpdateChecked2 }),
        key: '消费信息',
        label: '消费信息',
        type: undefined,
        disabled: true,
      ...
      {
        children: undefined,
        icon: () => h(Checkbox, { checked: checked1, 'onUpdate:checked': onUpdateChecked1 }),
        key: '销售信息',
        label: '销售信息',
        type: undefined,
        disabled: true,
...
function handleCloseHeader(item) {
  headerList.value = headerList.value.filter((it) => it['data-id'] !== item['data-id'])
  updateSelectStyle()
  handleData()
}
...
function handleRowDrop() {
  if (drag === 'bigClass' && rowList.value.every((item) => item['data-id'] !== 'bigClass')) {
...
function handleColDrop() {
  if (drag === 'area' && colList.value.every((item) => item['data-id'] !== 'area')) {
    const result = menus.value
      .map((item) => [...item.children])
      .flat(Infinity)
...
async function handleData() {
  ...
  console.log('if', rowList.value.length && colList.value.length & headerList.value.length)
  ...
  if (rowList.value.length && colList.value.length && headerList.value.length) {
    ...
    if (rowList.value.length === 1 && rowList.value[0]['data-id'] === 'bigClass') {
      gridOptions.data = handleGoodsList(await getGoodsList()).filter(
        (item) => item.mediumClass === '中类-合计' || item.mediumClass === '',
      )
    } else if (rowList.value.length === 1 && rowList.value[0]['data-id'] === 'mediumClass') {
      gridOptions.data = handleGoodsList(await getGoodsList()).filter(
...
function handleGoodsList(goodsList) {
  ...
  const mediumClass = [...new Set(goodsList.map((item) => item.mediumClass))].map((item) =>
    goodsList.find((it) => it.mediumClass === item),
  )
  ...
  let result = mediumClass.map((row) => {
    ...
    return {
      ...
      huanan: goodsList
        .filter((item) => item.area === '华南' && item.mediumClass === row.mediumClass)
        .reduce((prev, curr) => {
      ...
      huabei: goodsList
        .filter((item) => item.area === '华北' && item.mediumClass === row.mediumClass)
        .reduce((prev, curr) => {
          prev['salesVolume'] = curr.salesVolume
          prev['totalSales'] = curr.totalSales
          prev['refundAmount'] = curr.refundAmount
      ...
      huadong: goodsList
        .filter((item) => item.area === '华东' && item.mediumClass === row.mediumClass)
        .reduce((prev, curr) => {
          prev['salesVolume'] = curr.salesVolume
          prev['totalSales'] = curr.totalSales
          prev['refundAmount'] = curr.refundAmount
  ...
  result = [...new Set(result.map((item) => item.bigClass))]
    .map((bigClass) => {
      ...
      const last = tmp.reduce((prev, curr) => {
        ...
        prev['huadong'] = {
          ...curr.huadong,
          salesVolume: Number(curr?.huadong?.salesVolume) + Number(prev?.huadong?.salesVolume || 0),
          totalSales: Number(curr?.huadong?.totalSales) + Number(prev?.huadong?.totalSales || 0),
          refundAmount:
            Number(curr?.huadong?.refundAmount) + Number(prev?.huadong?.refundAmount || 0),
          associatedPurchaseRate: (Math.random() * 2).toFixed(1), // 连带率
        }

        prev['huabei'] = {
          ...curr.huabei,
          salesVolume: Number(curr.huabei.salesVolume) + Number(prev?.huabei?.salesVolume || 0),
          totalSales: Number(curr.huabei.totalSales) + Number(prev?.huabei?.totalSales || 0),
          refundAmount: Number(curr.huabei.refundAmount) + Number(prev?.huabei?.refundAmount || 0),
          associatedPurchaseRate: (Math.random() * 2).toFixed(1), // 连带率
        }

        prev['huanan'] = {
          ...curr.huanan,
          salesVolume: Number(curr.huanan.salesVolume) + Number(prev?.huanan?.salesVolume || 0),
          totalSales: Number(curr.huanan.totalSales) + Number(prev?.huanan?.totalSales || 0),
          refundAmount: Number(curr.huanan.refundAmount) + Number(prev?.huanan?.refundAmount || 0),
          associatedPurchaseRate: (Math.random() * 2).toFixed(1), // 连带率
        }
        return prev
      }, {})
  ...
  <div class="demo-page-wrapper">
    <div class="main">
      ...
      <div class="table">
        <div class="select">
          <div class="row-col">
            <div class="row" @dragover="(e) => e.preventDefault()" @drop="handleRowDrop">
              <div>行维度：</div>
              <a-tag
            ...
            <div class="col" @dragover="(e) => e.preventDefault()" @drop="handleColDrop">
              <div>列维度：</div>
              <a-tag
          ...
          <div class="table-header" @dragover="(e) => e.preventDefault()" @drop="handleHeaderDrop">
            <div>表头：</div>
            <a-tag
