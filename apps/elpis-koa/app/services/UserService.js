...
module.exports = app => {
  ...
  return class UserService extends BaseService {
    ...
    async create({ username, nickname, desc, sex }) {
      const userId = (global?.crypto?.randomUUID() || uuidv4())?.replace(/-/g, "");
    ...
    async findAll(condition) {
      ...
      condition.size = Number(condition.size) >= 1 ? Number(condition.size) : 10;

      const { username, nickname, sex, createTimeStart, createTimeEnd, page, size } = condition;
    ...
    async findTotal(condition) {
      const { username, nickname, sex, createTimeStart, createTimeEnd } = condition;
      const query = {};
      if (username) query.username = username;
      if (nickname) query.nickname = nickname;
      if (sex) query.sex = sex;
      // select count(distinct `user_id`) as `user_amount` from `user`
      let sql = app.knex("user").countDistinct("user_id as user_amount").where(query);
    ...
    async findOneByUsernameAndPassword({ username, password }) {
      ...
      const res = await app.knex("user").select("*").where({ username, password, isDelete: 1 }).limit(1); // 限制查一条
      return res[0];
    }
